...
fun processFieldGroup(fieldGroup: FieldGroup, container: TypeSpec.Builder, config: SuparnaturalGraphqlExtension, top: Boolean = false) {
    ...
    if (!hasFragments && !top) {
        ...
        if (hasFragments) {
            ...
            // adapter serializer
            val decoder = ParameterSpec.builder("decoder", Decoder::class).build()
            val encoder = ParameterSpec.builder("encoder", Encoder::class).build()
            val obj = ParameterSpec.builder("obj", fragmentsAdapterClassName).build()
            ...
            val deserializeFunSpec = FunSpec.builder("deserialize")
                    ...
                    .addStatement("val delegate = %T.nonstrict.fromJson(%T.serializer(), json)", Json::class.asClassName(), rootImplClassName)
            ...
            fieldGroup.fragmentSpreads.forEach {
                ...
                serializeFunSpec.addStatement("if (adapter.fragments.$fragmentSpreadPropertyName != null) {")
                        .addStatement("jsonObjects.add(%T.nonstrict.toJson(%T.Companion, adapter.fragments.$fragmentSpreadPropertyName).jsonObject)", Json::class.asClassName(), fragmentSpreadClassName)
                        .addStatement("}")

            }

            deserializeFunSpec.addStatement("return %T(delegate, $FragmentsGroup(${fieldGroup.fragmentSpreads.joinToString(", ") { it.decapitalize() }}))", fragmentsAdapterClassName)
            ...
            serializeFunSpec.addStatement("jsonObjects.map { it.jsonObject }.forEach {jsonObject ->")
                    ...
                    .addStatement("\t}")
                    .addStatement("}")
                    .addStatement("(encoder as %T).encodeJson(%T(jsonMap))", JsonOutput::class, JsonObject::class.asClassName())
            ...
            serializerSpec.addProperty(
                    PropertySpec.builder("descriptor", SerialDescriptor::class, KModifier.OVERRIDE)
                            .initializer(CodeBlock.builder().addStatement("%T.%M(\"$rootSpecTypeName\")", StringDescriptor::class.asClassName(), MemberName("kotlinx.serialization", "withName")).build())
                            .build()
...
//            rootSpec.addAnnotation(
//                    AnnotationSpec.builder(Serializable::class)
//                            .addMember("with = %T.%T.Companion::class", rootClassName, fragmentsAdapterClassName)
//                            .build()
//            )
        } else {
...
//                            .addMember("with = %T.%T.Companion::class", rootClassName, rootImplClassName)
//                            .build()
//            )
        }
                ...
                TypeSpec.companionObjectBuilder()
                        ...
                        .addFunction(
                                FunSpec
                                        ...
                                        .addStatement("return %T.serializer().serialize(encoder, obj as %T)", rootSerializerClassName, rootSerializerClassName)
                                        .build()
                        )
                        .addFunction(
                                FunSpec
...
fun processFragments(fragments: List<Fragment>, config: SuparnaturalGraphqlExtension): FileSpec {
    ...
    fragments.forEach { fragment ->
        println(fragment.fragmentSpreads)
